      ...
      <div className={styles.titleBar}>
        <div className={styles.titleBarContent}>
          {type == undefined ? (
            ...
            <Link className={styles.title} to={`/docs/${repository}`}>
              {repository}
            </Link>
          )}
          {type == undefined ? (
            <a
              href={repositoryJSON?.html_url}
              target={"__blank"}
              className={clsx("material-icons", "md-36", styles.titleBarIcon)}
            >
              open_in_new
            </a>
          ) : (
        ...
        <div className={styles.description}>
          {description ??
            repositoryJSON?.description?.replace(
              /([\uE000-\uF8FF]|\uD83C[\uDF00-\uDFFF]|\uD83D[\uDC00-\uDDFF])/g,
              ""
            )}
        </div>
          ...
          renderArrowPrev={(clickHandler, hasPrev, label) => {
            return (
              <span className="arrow-left" onClick={clickHandler}>
                <span className={clsx("material-icons", styles.carouselIcon)}>
                  arrow_left
                </span>
              </span>
            );
        ...
        >
          {images.map((e) => (
            <div className={styles.image}>
              <img loading="lazy" className={styles.imageChild} src={e}></img>
            </div>
          ))}
        </Carousel>
      ) : null}
